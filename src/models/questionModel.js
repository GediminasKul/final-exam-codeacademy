...
//   const sql = `UPDATE questions SET archived=1 WHERE id=${id.id} AND user_id=${id.user_id}`;
//   return executeDb(sql, [id, user_id]);
// }

function getAnswersFromDb(id) {
  const sql = `SELECT * FROM replies WHERE question_id=${id.id} AND archived=0`;
  return executeDb(sql, [id]);
}

function saveAnswerToDb(user_id, question_id, content) {
  const sql =
    'INSERT INTO replies (user_id, question_id, content) VALUES (?, ?, ?)';
